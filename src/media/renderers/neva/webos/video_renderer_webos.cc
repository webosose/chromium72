...
void VideoRendererWebOS::Flush(const base::Closure& callback) {
  ...
  video_decoder_stream_->Reset(
      base::Bind(&VideoRendererWebOS::OnVideoDecoderStreamResetDone,
                 weak_factory_.GetWeakPtr()));

  // To avoid unnecessary work by VDAs, only delete queued frames after
  // resetting |video_decoder_stream_|. If this is done in the opposite order VDAs
...
void VideoRendererWebOS::Initialize(
  ...
  video_decoder_stream_->Initialize(
      stream, base::Bind(&VideoRendererWebOS::OnVideoDecoderStreamInitialized,
                         weak_factory_.GetWeakPtr()),
...
void VideoRendererWebOS::OnVideoDecoderStreamInitialized(bool success) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  base::AutoLock auto_lock(lock_);
...
void VideoRendererWebOS::FrameReady(VideoDecoderStream::Status status,
  ...
  } else if ((low_delay_ || !video_decoder_stream_->CanReadWithoutStalling()) &&
...
void VideoRendererWebOS::OnVideoDecoderStreamResetDone() {
...
bool VideoRendererWebOS::IsBeforeStartTime(base::TimeDelta timestamp) {
  return timestamp + video_decoder_stream_->AverageDuration() < start_timestamp_;
}

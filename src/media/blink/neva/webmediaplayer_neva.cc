...
void WebMediaPlayerNeva::OnDidCommitCompositorFrame() {
  ...
  if (!UpdateBoundaryRectangle()) {
    // UpdateBoundaryRectangle fails when video layer is not in current
    // composition.
    if (HasVisibility()) {
      is_video_offscreen_ = true;
      SetVisibility(false);

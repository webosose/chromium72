...
void URLRequestContextFactory::InitializeSystemContextDependencies() {
  ...
  ssl_config_service_ = std::make_unique<SSLConfigServiceTLS13>();
  ...
  if (proxy_settings.enabled) {
    ...
    if (!proxy_bypass_list.empty()) {
      ...
      proxy_config.proxy_rules()
          .bypass_rules.ParseFromString(proxy_bypass_list, net::ProxyBypassRules::ParseFormat::kHostnameSuffixMatching);
    }

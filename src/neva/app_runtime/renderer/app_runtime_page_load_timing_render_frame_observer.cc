...
void AppRuntimePageLoadTimingRenderFrameObserver::DidChangePerformanceTiming() {
  ...
  if (PageLoadTimingIsFirstTextPaint()) {
    mojom::AppRuntimeWebViewHostAssociatedPtr interface;
    render_frame()->GetRemoteAssociatedInterfaces()->GetInterface(&interface);
    if (interface)
      interface->DidFirstTextPaint();
  }
...
bool AppRuntimePageLoadTimingRenderFrameObserver::
    PageLoadTimingIsFirstTextPaint() {
  if (first_text_paint_)
    return false;
  ...
  if (perf.FirstTextPaint() > 0.0) {
    first_text_paint_ =
        ClampDelta(perf.FirstTextPaint(), perf.NavigationStart());
    return true;
  }
  return false;
}

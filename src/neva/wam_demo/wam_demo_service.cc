...
char kFirstTextPaintTime[] = "firstTextPaintTime";
...
void WamDemoService::Launch(const std::string& appid, const std::string& appurl,
  ...
  BlinkView* webpage = new BlinkView(params.width, params.height, this, profile);
...
void WamDemoService::DataUpdated(const std::string& url,
  ...
  if (app_it != appslist_.end()) {
    ...
    if (cmd == command::kChangeUrl) {
      ...
      base::FilePath path = webpage->GetWebContents()->GetBrowserContext()->GetPath();
      command += ':' + path.BaseName().value() + ':' +
...
void WamDemoService::OnPerformanceEvent(app_runtime::WebViewBase* view,
  ...
  if (app_it != appslist_.end()) {
    ...
    LOG(INFO) << "Application " << app_it->appid_ << " " << type << " is " <<
        delta.InMilliseconds() << " (ms)";
    std::string command = type;
    command += ":";
    command += base::Int64ToString(delta.InMilliseconds());
    EmulatorSendData(command, app_it->appid_);
  }
}

void WamDemoService::OnDidLoadingEnd(app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kLoadingEndTime);
}

void WamDemoService::OnDidFirstPaint(app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstPaintTime);
}

void WamDemoService::OnDidFirstContentfulPaint(
    app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstContentfulPaintTime);
}

void WamDemoService::OnDidFirstTextPaint(app_runtime::WebViewBase* window) {
  OnPerformanceEvent(window, response::kFirstTextPaintTime);
}

void WamDemoService::OnDidFirstImagePaint(app_runtime::WebViewBase* window) {
  ...
  if (UnpackString(value, argument::kGroupName, group_name)) {
    ...
    if (UnpackBool(value, argument::kIsOwner, is_owner)) {
      if (is_owner) {
        ...
        app_runtime::WindowGroupConfiguration config;
